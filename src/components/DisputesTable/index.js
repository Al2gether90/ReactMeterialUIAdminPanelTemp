...
const DisputesTable = ({ disputes, columns, onRowClick }) => {
  ...
  useEffect(() => {
    setRows(disputes.filter((dispute) => dispute.product.name.toLowerCase().indexOf(keyword.toLowerCase()) > -1
      || dispute.product.price.toString().indexOf(keyword.toLowerCase()) > -1
      || dispute.status.toLowerCase().indexOf(keyword.toLowerCase()) > -1));
    setPage(0);
  }, [disputes, keyword]);
    ...
    <Paper>
      <TableContainer className="disputes-table">
        ...
        <Table>
          ...
          <TableBody>
            {rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((dispute) => (
              <TableRow hover tabIndex={-1} key={dispute.id} onClick={() => onRowClick(dispute.id)}>
                {columns.map((column, index) => {
